...
## 📦 MegaProject wiring (read this first)

This copy is wired into the **Super Bot platform** (see the root [`README.md`](../README.md)).
What's specific here:

- **Talks straight to the LangGraph server at `:2024`** — `NEXT_PUBLIC_API_URL` in
  [`.env.local`](.env.local) points there, so chat streams via `useStream`
  ([`src/providers/Stream.tsx`](src/providers/Stream.tsx)) directly, bypassing the
  Next.js proxy route. (The FastAPI gateway on `:8000` is for control/REST + PDF upload,
  not the streaming chat.)
- **The agent dropdown** ([`src/components/agent-switcher.tsx`](src/components/agent-switcher.tsx))
  sets the `assistantId` query param and clears the thread. Each `id` **must match a graph
  id in `backend/langgraph.json`**. Current list: `superbot` (default, auto-routes),
  `personal_chef`, `email_agent`, `wedding_planner`, `pdf_chatbot`, `movie_recommender`.
- **Adding an agent?** After registering it in the backend, add one entry to the `AGENTS`
  array in `agent-switcher.tsx`. That's the only frontend change. See the root README's
  "Adding an agent" recipe.
- **PDF Chatbot:** attaching a PDF in chat (paperclip / drag-drop) works for RAG — a
  `before_agent` middleware in the backend embeds it into the knowledge base before the
  agent runs. (`POST :8000/upload` also exists for batch/API ingestion.)

The rest of this file is the upstream Agent Chat UI documentation.
